 ...
 * =====================================================================================
 *
 *       Filename:  my_non_overlap_OASG.cpp
 *
 *    Description:   哇 ， 這個名字雖然是取 my_non_overlap_OASG , 
 *    		    但是  ， 我要建的overlap_OASG , 看來也是要放在這裡了 
 *
 *        Version:  1.0
 *        Created:  2011/02/08 15時17分28秒
 *       Revision:  none
 *       Compiler:  gcc
 *
...
#ifndef         MY_NON_OVERLAP_OASG_H
	#define 	MY_NON_OVERLAP_OASG_H
#include	<iostream>
#include	<fstream>
#include	"edge.h"
#include	"obstacle.h"
...
using namespace std ; 
 ...
 public:
 	my_non_overlap_OASG(){
		...
		/*-----------------------------------------------------------------------------
		 *  
		op_x = &my_non_overlap_OASG::sort_x_position_in_OASG ; 
		op_y = &my_non_overlap_OASG::sort_y_position_in_OASG ; 
		幹 ， 完全不行
		 *-----------------------------------------------------------------------------*/
	}
 	// 這裡我想先宣告 ， 下面再放實作 
...
bool (*sort_x_position) (vertex , vertex )  , bool (*sort_y_position)  (vertex , vertex)
 *-----------------------------------------------------------------------------*/
void my_non_overlap_OASG::build_OASG( vector<edge> &OASG , level_info &my_level_info ){	
		 ...
		 *-----------------------------------------------------------------------------*/
		for(int i = 0 ; i < my_level_info.plane_width ; i++){
			sort( x_position[i].begin() , x_position[i].end() , sort_y_position_in_OASG  );
			// 按  自已寫個sort 好了 
		...
		// 這裡為什麼 要sort y 我有點忘了  
		for(int i = 0 ; i < my_level_info.plane_height; i++){
			 sort( y_position[i].begin() , y_position[i].end() , sort_x_position_in_OASG );
		}			
		...
		/*-----------------------------------------------------------------------------
		 *   先把這區塊的東西拿起來看看  ，   先照 overlap 的樣子去弄   ,  我想呢  ， 出錯是因為
		 *   我寫 overlap 時改到了  一些這裡會用到的東西 
			...
			if( x_position[i].size() == 1 ){
				...
				for(unsigned int j = 0 ; j < x_position[i].size()  ; j++  ){
					...
					// normal case 
						...
						if( x_position[i][j].print_corner_id() == 1 ){
		...
		for(int i = 0 ; i < my_level_info.plane_width ; i++  ){
			...
			for(unsigned int j = 0 ; j < x_position[i].size()  ; j++  ){
				...
				if( x_position[i][j].core_state() == true ){
					...
					else if(  x_position[i][j].print_corner_id() == 1 || x_position[i][j].print_corner_id() == 4 ){
						  x_position[i][j].set_r_interval( r ) ;	 
					}
					else if(  x_position[i][j].print_corner_id() == 2 || x_position[i][j].print_corner_id() == 3 ){
						  x_position[i][j].set_l_interval( l ) ;	 
					}
		...
		for(unsigned int j = 0 ; j < OASG.size() ; j++  ){
			OASG[j].print_edge_information_for_gnu() ; 
		} 
		 *-----------------------------------------------------------------------------*/
		...
		do{
			...
			how_to_move = x_position[interval_line][h].modify_interval_and_add_edgs_to_OASG( Segment[segment_line][k].print_seg_state() , segment_line ,
											   Segment[segment_line][k].print_left_of_seg(), 
											   Segment[segment_line][k].print_right_of_seg(),
											   OASG   ,my_level_info.plane_height - 1 , head_tail_state[segment_line][0], 
											   head_tail_state[segment_line][1] , 0 ); 	
				//下面這個是要印plane頭尾　，　是否有vertex 
				//cout << "x = " << segment_line << " , (" << head_tail_state[segment_line][0] << " , " << head_tail_state[segment_line][1] << " ) "
				//<< endl ;     
 ...
 *-----------------------------------------------------------------------------*/
void my_non_overlap_OASG::build_OASG_can_handle_projection_vertex( level_info &my_level_info ){	
		...
		for(int i = 0 ; i < my_level_info.plane_width ; i++ ){
			...
			for(unsigned int j = 0  ; j < x_position[i].size() ; j++  ){
				...
				// 下面這個 ix 是新加的 ， 要能有抓到 projection_vertex , 因為 projection_vertex 的設定就是  false 及 0 
		...
		for(int i = 0 ; i < my_level_info.plane_width ; i++  ){
			...
			for(unsigned int j = 0 ; j < x_position[i].size()  ; j++  ){
				...
				if( x_position[i][j].core_state() == true ){
					...
					else if(  x_position[i][j].print_corner_id() == 0  ){
						  x_position[i][j].set_l_interval( l ) ;	 
						  x_position[i][j].set_r_interval( r ) ;	 
				...
				if(  x_position[i].size() > 1 ){
					...
					if( j <= x_position[i].size() - 2 && ( x_position[i][j].Is_pin_or_not() != true || 
						...
						for(unsigned int k = 0 ; k < my_level_info.complete_obstacle_information.size() ; k++  ){
							...
							if( i < my_level_info.complete_obstacle_information[k].print_ru_corner_x() && 
							    i > my_level_info.complete_obstacle_information[k].print_ld_corner_x() &&  
							    x_position[i][j].print_y_position() > my_level_info.complete_obstacle_information[k].print_ld_corner_y() && 
							    x_position[i][j+1].print_y_position() < my_level_info.complete_obstacle_information[k].print_ru_corner_y() ){
								close_case = true ; 	 
								break ; 
							}
						...
						for(unsigned int k = 0 ; k < my_level_info.complete_obstacle_information.size() ; k++  ){
							//先判斷x , 看有沒有在肚子　	 
							if( i == my_level_info.complete_obstacle_information[k].print_ld_corner_x() &&  
							    x_position[i][j].print_y_position() <=  my_level_info.complete_obstacle_information[k].print_ru_corner_y() && 
							    x_position[i][j+1].print_y_position() >= my_level_info.complete_obstacle_information[k].print_ld_corner_y() ){
								anothor_case = true ; 	 
								//有抓到就不用一直比下去了
								break ; 
							}
 ...
 *  
 ...
 裡面一直抓不到我要的東西
 *-----------------------------------------------------------------------------*/
void my_non_overlap_OASG::print_OASG_gnu( vector<edge> &OASG , level_info &my_level_info ){
	...
	oasgcout.close() ;
}// end of print_OASG_gnu


/*-----------------------------------------------------------------------------
 *   這裡要印出整個  3D 的 OASG 
 *-----------------------------------------------------------------------------*/
void my_non_overlap_OASG::print_3D_OASG_gnu( vector<level_info> &every_level_info , vector<edge> &vertical_edge ){
